...
# Asegurar que existe la columna categoria
...
def parsear_codigo_bascula(codigo_completo):
    """
    Parsear códigos de barras de la báscula que pueden contener múltiples productos.
    
    Formatos de ticket:
    - Productos a granel: 8 dígitos de código + 4 dígitos de peso en gramos + 1 dígito de control (13 dígitos total)
      Ejemplo: 20001400 18758 -> Código: 20001400, Peso: 1875g (1.875kg), Control: 8 (descartado)
    - Productos por unidad: 8 dígitos de código + 1 dígito de cantidad (9 dígitos total)
    
    Retorna lista de tuplas: [(codigo, cantidad/peso), ...]
    """
    if not codigo_completo or len(codigo_completo) < 9:
        return [(codigo_completo, None)]
    
    productos_parseados = []
    i = 0
    
    while i < len(codigo_completo):
        # Intentar parsear como producto a granel (13 dígitos: 8 código + 4 peso + 1 control)
        if i + 13 <= len(codigo_completo):
            posible_codigo_granel = codigo_completo[i:i+8]  # Primeros 8 dígitos
            posible_peso = codigo_completo[i+8:i+12]  # Siguientes 4 dígitos (peso en gramos)
            # El dígito i+12 es el de control y se descarta
            
            # Verificar si existe este código en la base de datos como producto a granel
            cursor.execute("""
                SELECT codigo, tipo_venta FROM productos 
                WHERE codigo = ? AND tipo_venta = 'granel'
            """, (posible_codigo_granel,))
            resultado_granel = cursor.fetchone()
            
            if resultado_granel:
                # Es un producto a granel válido
                try:
                    gramos = int(posible_peso)
                    peso_kg = gramos / 1000.0  # Convertir gramos a kilogramos
                    productos_parseados.append((posible_codigo_granel, peso_kg))
                    i += 13  # Avanzar 13 posiciones
                    continue
                except ValueError:
                    pass
        
        # Intentar parsear como producto por unidad (13 dígitos: 8 código + 4 relleno + 1 cantidad)
        if i + 13 <= len(codigo_completo):
            posible_codigo_unidad = codigo_completo[i:i+8]
            posible_cantidad = codigo_completo[i+8:i+9]  # Noveno dígito es la cantidad
            
            # Verificar si existe este código en la base de datos como producto por unidad
            cursor.execute("""
                SELECT codigo, tipo_venta FROM productos 
                WHERE codigo = ? AND tipo_venta = 'unidad'
            """, (posible_codigo_unidad,))
            resultado_unidad = cursor.fetchone()
            
            if resultado_unidad:
                # Es un producto por unidad válido
                try:
                    cantidad = int(posible_cantidad)
                    # Si la cantidad es 0, usar 1 por defecto
                    if cantidad == 0:
                        cantidad = 1
                    productos_parseados.append((posible_codigo_unidad, cantidad))
                    i += 13  # Avanzar 13 posiciones (formato completo)
                    continue
                except ValueError:
                    pass
        
        # Si no se pudo parsear, avanzar 1 posición
        i += 1
    
    # Si no se parseó nada, retornar el código original
...
def obtener_producto_por_codigo(codigo):
    """Obtener información completa del producto por código o nombre (búsqueda flexible)"""
    if not codigo:
        return None
    
    # Primero buscar por código exacto (case-insensitive)
    cursor.execute("SELECT * FROM productos WHERE LOWER(codigo) = LOWER(?)", (codigo,))
    resultado = cursor.fetchone()
    
    # Si no se encuentra por código, buscar por nombre flexible
    if not resultado:
        cursor.execute("""
...
def mostrar():
    ...
    # JavaScript para autoenfoque usando components.html
    components.html(
        """
        <script>
        ...
        function focusCodigoBarras() {
            ...
            if (iframe) {
                ...
                inputs.forEach(input => {
                    ...
                    if (placeholder && (placeholder.includes('código de barras') || placeholder.includes('Escanea'))) {
                        input.focus();
                        input.select();
                        console.log('Input de código de barras enfocado');
                    }
                });
            }
        }
        
        // Ejecutar múltiples veces para asegurar el enfoque
        setTimeout(focusCodigoBarras, 100);
        setTimeout(focusCodigoBarras, 300);
        setTimeout(focusCodigoBarras, 500);
        setTimeout(focusCodigoBarras, 800);
        setTimeout(focusCodigoBarras, 1000);
        </script>
        """,
        height=0,
    )
    
    # Detectar si es un código de báscula (múltiples productos concatenados)
    productos_parseados = []
    if codigo and len(codigo) >= 13:
        # Intentar parsear como código de báscula
        productos_parseados = parsear_codigo_bascula(codigo)
        
        # Verificar si se parsearon múltiples productos o un producto con peso/cantidad
        if len(productos_parseados) > 1 or (len(productos_parseados) == 1 and productos_parseados[0][1] is not None):
            # Agregar automáticamente los productos parseados al carrito
            if 'ultimo_codigo_parseado' not in st.session_state or st.session_state.ultimo_codigo_parseado != codigo:
                # Marcar este código como procesado para evitar duplicados
                st.session_state.ultimo_codigo_parseado = codigo
                
                # Variables por defecto para el flujo inicial
                cliente_tipo_inicial = "Normal"
                
                # Procesar múltiples productos de la báscula
                productos_agregados = 0
                for codigo_prod, cantidad_peso in productos_parseados:
                    producto_temp = obtener_producto_por_codigo(codigo_prod)
                    if producto_temp:
                        info_temp = obtener_informacion_producto(producto_temp)
                        
                        if info_temp['tipo_venta'] == 'granel':
                            # Producto a granel - cantidad_peso está en Kg
                            peso = cantidad_peso
                            if info_temp['stock_kg'] >= peso:
                                precio_kg = obtener_precio_granel_por_tipo(producto_temp, cliente_tipo_inicial)
                                total = precio_kg * peso
                                
                                st.session_state.carrito.append({
                                    'codigo': info_temp['codigo'],
                                    'nombre': f"{info_temp['nombre']} ({peso:.3f} Kg)",
                                    'cantidad': 1,
                                    'peso': peso,
                                ...
                                st.error(f"❌ Stock insuficiente para {info_temp['nombre']}. Disponible: {info_temp['stock_kg']:.3f} Kg")
                        
                    else:
                            # Producto por unidad - cantidad_peso es la cantidad
                            cantidad = int(cantidad_peso)
                            if info_temp['stock'] >= cantidad:
                                # Obtener precio del producto por unidad usando el precio_normal del info_producto
                                precio = info_temp['precio_normal']
                                
                                # Si el precio es 0, intentar obtenerlo de la función
                                if precio == 0:
                                    precio = obtener_precio_por_tipo(producto_temp, cliente_tipo_inicial)
                                
                                total = precio * cantidad
                                
                                st.session_state.carrito.append({
                                    'codigo': info_temp['codigo'],
                                    'nombre': f"{info_temp['nombre']}",
                                    'cantidad': cantidad,
                                    'peso': 0,
                                ...
                                st.error(f"❌ Stock insuficiente para {info_temp['nombre']}. Disponible: {info_temp['stock']} unidades")
                
                if productos_agregados > 0:
                    st.session_state.scroll_to_carrito = True
                    st.rerun()
            
            # Mostrar información de productos parseados
            st.markdown("""
            <div style="background: linear-gradient(135deg, #a29bfe 0%, #6c5ce7 100%); padding: 1.5rem; border-radius: 15px; margin: 1rem 0; color: white; text-align: center;">
                <div style="font-size: 1.6rem; font-weight: bold; margin-bottom: 0.5rem;">
                    ⚖️ CÓDIGO DE TICKET DETECTADO
                </div>
            </div>
            """, unsafe_allow_html=True)
            
            # Mostrar información de depuración del código escaneado
            if len(codigo) == 13:
                codigo_producto = codigo[0:8]
                peso_str = codigo[8:12]
                digito_control = codigo[12]
                st.info(f"🔍 **Análisis del código:** `{codigo}`")
                st.caption(f"• **Código producto:** `{codigo_producto}` (8 dígitos)")
                st.caption(f"• **Peso en gramos:** `{peso_str}` = {int(peso_str)/1000:.3f} kg")
                st.caption(f"• **Dígito control:** `{digito_control}` (descartado)")
            
            # Mostrar cada producto parseado
            for idx, (codigo_prod, cantidad_peso) in enumerate(productos_parseados):
                producto_info_temp = obtener_producto_por_codigo(codigo_prod)
                if producto_info_temp:
                    info_temp = obtener_informacion_producto(producto_info_temp)
                    if info_temp['tipo_venta'] == 'granel':
                        st.success(f"✅ Producto {idx+1}: **{info_temp['nombre']}** - Peso: **{cantidad_peso:.3f} Kg** - AGREGADO AL CARRITO")
                else:
                        st.success(f"✅ Producto {idx+1}: **{info_temp['nombre']}** - Cantidad: **{int(cantidad_peso)} unidades** - AGREGADO AL CARRITO")
            else:
                    st.error(f"❌ Producto {idx+1}: Código **{codigo_prod}** no encontrado en la base de datos")
                st.warning(f"💡 Verifica que el producto con código `{codigo_prod}` esté registrado como 'granel' en el sistema")
    
    # Verificar si el producto existe y obtener información (SOLO para códigos normales)
    producto_info = None
    info_producto = None
    
    # Solo buscar como producto individual si NO se parsearon productos de báscula
    if codigo and not productos_parseados:
        # Código normal (no de báscula)
        producto_info = obtener_producto_por_codigo(codigo)
        if producto_info:
            info_producto = obtener_informacion_producto(producto_info)
    
    # Solo mostrar mensaje de "producto no encontrado" si NO hay productos parseados
    if not productos_parseados or (len(productos_parseados) == 1 and productos_parseados[0][1] is None):
    ...
    with col_btn_agregar[1]:
        if st.button("🛒 AGREGAR AL CARRITO", type="primary"):
            # Verificar si hay productos parseados de la báscula
            if productos_parseados and len(productos_parseados) > 0:
                ...
                if productos_agregados > 0:
                    st.markdown(f"""
                    <div style="background: linear-gradient(135deg, #00b894 0%, #00cec9 100%); padding: 1.5rem; border-radius: 15px; text-align: center; color: white; font-size: 1.2rem; font-weight: bold; margin: 1rem 0;">
                        ✅ SE AGREGARON {productos_agregados} PRODUCTO(S) AL CARRITO
                    </div>
                    """, unsafe_allow_html=True)
                ...
                # Producto individual normal (no de báscula)
                if info_producto['tipo_venta'] == 'granel':
    ...
    if st.session_state.carrito:
        st.markdown("---")
        st.markdown("""        
        <div id="carrito-compras" style="background: linear-gradient(135deg, #a8d5ba 0%, #90c695 100%); padding: 1.5rem; border-radius: 16px; margin: 0.8rem 0; border: 2px solid #7eb693;">
            <h2 style="color: #1e3a28; text-align: center; margin-bottom: 0.8rem; font-size: 1.6rem;">🛒 CARRITO DE COMPRAS</h2>
        </div>
        """, unsafe_allow_html=True)
        
        # Si se acaba de agregar productos automáticamente, hacer scroll al carrito
        if st.session_state.get('scroll_to_carrito', False):
            st.markdown("""
            <script>
                setTimeout(function() {
                    const carritoElement = document.getElementById('carrito-compras');
                    if (carritoElement) {
            ...
            """, unsafe_allow_html=True)
            st.session_state.scroll_to_carrito = False
        ...
        # Configuración de venta - DESPUÉS de agregar productos
        st.markdown("---")
        st.markdown("""
        <div style="background: linear-gradient(135deg, #2eb8b8 0%, #0984e3 100%); padding: 1rem; border-radius: 12px; margin: 0.5rem 0;">
            <h2 style="color: white; text-align: center; margin-bottom: 0; font-size: 1.4rem;">💰 PASO 2: CONFIGURAR VENTA</h2>
        </div>
        """, unsafe_allow_html=True)
        
        # Layout: Tipo de Cliente más pequeño a la izquierda
        col_cliente, col_espacio = st.columns([1, 3])
        ...
        # Tipos de Pago - Todo a lo largo
        st.markdown("#### 💳 Tipos de Pago")
        st.write("Modo de pago:")
        
        # Radio buttons con mismo diseño de checkboxes - en horizontal
        col_radio1, col_radio2, col_radio3, col_radio4 = st.columns([1, 1, 2, 2])
        with col_radio1:
            pago_unico = st.checkbox("💵 Pago único", value=True, key="pago_unico_check")
        with col_radio2:
            pago_mixto = st.checkbox("💰 Pago mixto", value=False, key="pago_mixto_check")
        
        # Lógica para que solo uno esté activo
        if pago_unico and pago_mixto:
            pago_mixto = False
        elif not pago_unico and not pago_mixto:
            pago_unico = True
        
        modo_pago = "💵 Pago único" if pago_unico else "💰 Pago mixto"
        
        if modo_pago == "💵 Pago único":
            st.markdown("**<span style='font-size: 1.15rem;'>Selecciona UN solo tipo de pago:</span>**", unsafe_allow_html=True)
            
            # Checkboxes en formato horizontal
            col_pago1, col_pago2, col_pago3, col_pago4 = st.columns(4)
            with col_pago1:
                pago_efectivo = st.checkbox("💵 Efectivo", value=True, key="efectivo_unico_final")
            with col_pago2:
                pago_tarjeta = st.checkbox("💳 Tarjeta", key="tarjeta_unico_final")
            with col_pago3:
                pago_transferencia = st.checkbox("📱 Transferencia", key="transferencia_unico_final")
            with col_pago4:
                pago_credito = st.checkbox("📋 Crédito", key="credito_unico_final")
            
            # Lógica para pago único: solo uno puede estar seleccionado
            opciones_seleccionadas = sum([pago_efectivo, pago_tarjeta, pago_transferencia, pago_credito])
            
            if opciones_seleccionadas > 1:
                st.warning("⚠️ Solo puedes seleccionar UN tipo de pago")
                pago_efectivo = True
                pago_tarjeta = False
                pago_transferencia = False
                pago_credito = False
            elif opciones_seleccionadas == 0:
                pago_efectivo = True
                pago_tarjeta = False
                pago_transferencia = False
                pago_credito = False
            ...
            st.markdown("**<span style='font-size: 1.15rem;'>Selecciona múltiples tipos de pago:</span>**", unsafe_allow_html=True)
            
            # Checkboxes en formato horizontal
            col_pago1, col_pago2, col_pago3, col_pago4 = st.columns(4)
            with col_pago1:
                pago_efectivo = st.checkbox("💵 Efectivo", value=True, key="efectivo_mixto_final")
            with col_pago2:
                pago_tarjeta = st.checkbox("💳 Tarjeta", key="tarjeta_mixto_final")
            with col_pago3:
                pago_transferencia = st.checkbox("📱 Transferencia", key="transferencia_mixto_final")
            with col_pago4:
                pago_credito = st.checkbox("📋 Crédito", key="credito_mixto_final")
        ...
        if cliente_tipo != "Normal":
            ...
            if descuentos_aplicados:
                st.markdown(f"""
                <div style="background: linear-gradient(135deg, #fdcb6e 0%, #e17055 100%); padding: 1rem; border-radius: 12px; text-align: center; color: white; font-size: 1.1rem; font-weight: bold; margin: 0.5rem 0;">
                    🎉 CLIENTE {cliente_tipo.upper()} - ¡{len(descuentos_aplicados)} DESCUENTOS APLICADOS! 🎉
                </div>
                """, unsafe_allow_html=True)
        ...
        if tipos_pago_seleccionados:
            st.markdown("---")
            st.markdown("""
            <div style="background: linear-gradient(135deg, #6c5ce7 0%, #a29bfe 100%); padding: 0.8rem; border-radius: 12px; margin: 0.5rem 0;">
                <h2 style="color: white; text-align: center; margin-bottom: 0; font-size: 1.3rem;">💳 DISTRIBUCIÓN DE PAGOS</h2>
            </div>
            """, unsafe_allow_html=True)
            ...
            if len(tipos_pago_seleccionados) == 1:
                ...
                st.markdown(f"""
                <div style="background: linear-gradient(135deg, #00cec9 0%, #55a3ff 100%); padding: 1rem; border-radius: 12px; text-align: center; color: white; font-size: 1.1rem; font-weight: bold; margin: 0.5rem 0;">
    ...
    # JavaScript adicional para mantener el foco en el campo de código de barras
    components.html(
        """
        <script>
        // Función para mantener el foco en el input de código de barras
        function mantenFocoCodigoBarras() {
            const iframe = window.frameElement;
            if (iframe) {
                const iframeDoc = iframe.ownerDocument;
                const inputs = iframeDoc.querySelectorAll('input[type="text"]');
                ...
                inputs.forEach(input => {
                    const placeholder = input.getAttribute('placeholder');
                    if (placeholder && (placeholder.includes('código de barras') || placeholder.includes('Escanea'))) {
                        // Verificar si no hay otro elemento activo importante
                        ...
                        const isButtonActive = activeElement && activeElement.tagName === 'BUTTON';
                        
                        // Solo enfocar si no hay otro input, textarea o botón activo
                        if (!isInputActive && !isTextareaActive && !isButtonActive) {
                            input.focus();
                        }
                    }
                });
        ...
        // Mantener el foco periódicamente
        setInterval(mantenFocoCodigoBarras, 2000);
        </script>
        """,
        height=0,
    )
