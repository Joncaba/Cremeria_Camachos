...
def parsear_codigo_bascula(codigo_completo):
    """
    Parsear códigos de barras de tickets que pueden contener múltiples productos.
    
    Formato por producto: 13 dígitos
    - 8 dígitos: código del producto
    - 4 dígitos: valor (gramos para granel, o cantidad codificada para unidad)
    - 1 dígito: control (se descarta)
    
    Ejemplo: 200014001875820004010200012000105031051
    - Producto 1: 20001400 + 1875 -> 1.875 kg granel
    - Producto 2: 20004010 + 0200 -> 2 unidades (0200 / 100 = 2)
    - Producto 3: 12000105 + 0310 -> depende del tipo
    
    Retorna lista de tuplas: [(codigo_producto, valor_bruto), ...]
    """
    if not codigo_completo or len(codigo_completo) < 13:
        return []
    
    # Verificar que la longitud sea múltiplo de 13
    ...
    for i in range(num_productos):
        inicio = i * 13
        codigo_producto = codigo_completo[inicio:inicio+8]
        valor_str = codigo_completo[inicio+8:inicio+12]
        # El dígito 12 es de control y se descarta
        
        try:
            valor = int(valor_str)
            productos_parseados.append((codigo_producto, valor))
        except ValueError:
            continue
...
def mostrar():
    ...
    # DETECCIÓN Y PROCESAMIENTO DE TICKETS
    # Detectar si es un ticket (múltiplo de 13 dígitos y al menos 13 caracteres)
    es_ticket = False
    if codigo and len(codigo) >= 13 and len(codigo) % 13 == 0:
        ...
        if productos_ticket:
            # Mostrar información del ticket detectado
            st.markdown(f"""
            <div style="background: linear-gradient(135deg, #6c5ce7 0%, #a29bfe 100%); padding: 1.5rem; border-radius: 15px; margin: 1rem 0; color: white; text-align: center;">
                ...
                <div style="font-size: 1.2rem;">
                    📦 {len(productos_ticket)} producto(s) encontrado(s) en el ticket
                </div>
            </div>
            """, unsafe_allow_html=True)
            
            # Botón para procesar ticket
            if st.button("✅ PROCESAR TICKET Y AGREGAR AL CARRITO", type="primary", width='stretch'):
                productos_agregados = 0
                productos_no_encontrados = []
                
                for codigo_prod, valor in productos_ticket:
                    # Buscar producto en la base de datos
                    producto_ticket = obtener_producto_por_codigo(codigo_prod)
                    
                    if producto_ticket:
                        # Obtener información del producto
                        info_prod = obtener_informacion_producto(producto_ticket)
                        
                        if info_prod['tipo_venta'] == 'granel':
                            # Para productos a granel: valor está en gramos
                            peso_kg = valor / 1000.0
                            precio_kg = obtener_precio_granel_por_tipo(producto_ticket, "Normal")
                            total = precio_kg * peso_kg
                            
                            if info_prod['stock_kg'] >= peso_kg:
                                st.session_state.carrito.append({
                                    'codigo': info_prod['codigo'],
                                    'nombre': f"{info_prod['nombre']} ({peso_kg:.3f} Kg)",
                                    'cantidad': 1,
                                    'peso': peso_kg,
                                ...
                                productos_no_encontrados.append(f"{info_prod['nombre']} (sin stock suficiente)")
                        
                        else:  # tipo_venta == 'unidad'
                            # Para productos por unidad: dividir valor entre 100
                            cantidad_unidades = valor // 100
                            if cantidad_unidades == 0:
                                cantidad_unidades = 1
                            
                            precio_unit = obtener_precio_por_tipo(producto_ticket, "Normal")
                            total = precio_unit * cantidad_unidades
                            
                            if info_prod['stock'] >= cantidad_unidades:
                                st.session_state.carrito.append({
                                    'codigo': info_prod['codigo'],
                        ...
                        productos_no_encontrados.append(f"Código {codigo_prod}")
                ...
                if productos_no_encontrados:
                    st.warning(f"⚠️ No se pudieron agregar: {', '.join(productos_no_encontrados)}")
                
                # Limpiar código y recargar
                if productos_agregados > 0:
                    st.rerun()
    
    # Verificar si el producto existe y obtener información (solo si NO es un ticket)
    producto_info = None
    info_producto = None
    ...
    if st.session_state.carrito:
        ...
        <div style="background: linear-gradient(135deg, #2eb8b8 0%, #0984e3 100%); padding: 1.5rem; border-radius: 16px; margin: 0.8rem 0;">
            <h2 style="color: white; text-align: center; margin-bottom: 0.8rem; font-size: 1.6rem;">💰 PASO 2: CONFIGURAR VENTA</h2>
        </div>
        """, unsafe_allow_html=True)
